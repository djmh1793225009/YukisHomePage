...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen relative flex flex-col items-center justify-start lg:justify-center p-4 sm:p-6 selection:bg-purple-500/30 overflow-x-hidden">
      ...
      <main className={`max-w-6xl w-full grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 lg:items-stretch transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'} py-8 lg:py-0`}>
        
        {/* Left Section: Personal Card */}
        <div className="lg:col-span-7">
          <div 
            className="group relative w-full h-full [perspective:2000px]"
            onMouseMove={handleMouseMove}
            onMouseLeave={() => setMousePos({ x: 0.5, y: 0.5 })}
          >
            <div 
              className="relative w-full h-full glass-card rounded-[32px] lg:rounded-[40px] p-6 sm:p-10 lg:p-12 transition-transform duration-300 ease-out shadow-2xl flex flex-col justify-between overflow-hidden group-hover:border-purple-500/40"
              style={{
                  ...
                  <div className="flex flex-wrap gap-1.5 lg:gap-2">
                    {USER_PROFILE.skills.map(skill => (
                      <div key={skill.name} className="flex flex-col gap-1 group/skill">
                        <div className={`px-2.5 py-1 rounded-lg border text-[9px] lg:text-[10px] font-bold uppercase tracking-wider transition-all ${
                          skill.name === 'More+' 
                          ? 'bg-purple-600/20 border-purple-500/50 text-purple-400 shadow-[0_0_10px_rgba(168,85,247,0.2)]' 
                          : 'bg-white/5 border-white/10 text-gray-500 group-hover/skill:border-purple-500/30 group-hover/skill:text-purple-400'
                        }`}>
                          {skill.name}
                        </div>
                      </div>
        ...
        <div ref={chatRef} className="lg:col-span-5 h-auto min-h-[400px] lg:min-h-0">
          {!showChat ? (
            <div className="h-full min-h-[400px] lg:min-h-0 glass-card rounded-[32px] lg:rounded-[40px] p-8 flex flex-col items-center justify-center text-center space-y-6 border-dashed border-white/10 border-2 group hover:border-purple-500/30 transition-all">
              <div className="relative">
